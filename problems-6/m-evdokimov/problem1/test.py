...
class Test_mixer(unittest.TestCase):
    ...
    def test_abc(self):
        word_mixer.mixer("Hello   world !", 'abc', how_to_print = custom_print)
        self.assertEqual(test_output, ['Hello', 'wlord', '!'])
        
    def test_short_words(self):
        word_mixer.mixer("Hey how are you ?", 'random', how_to_print = custom_print)
        self.assertEqual(test_output, ['Hey', 'how', 'are', 'you', '?'])
        
    def test_exeption(self):
        with self.assertRaises(ValueError):
            word_mixer.mixer("Hello world", 'bobr')
